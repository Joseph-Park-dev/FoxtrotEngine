...
void FileIOHelper::SaveVector4(std::ofstream& ofs, const std::string valName, const DirectX::XMFLOAT4& vec4)
{
	...
	std::string item =
		mItemIdent +
		"(" +
		std::to_string(vec4.x) + "," + std::to_string(vec4.y) + std::to_string(vec4.z) + "," + std::to_string(vec4.w) +
		")";
	mDataBuffer.push_back(itemTitle + item);
	++mItemCounts.back();
}
...
void FileIOHelper::SaveSize(std::ofstream& ofs, const std::string valName, const size_t& sizeVal)
{
	std::string itemTitle = mItemIdent + valName + "[size]" + '\n';
	std::string item = mItemIdent + std::to_string(sizeVal);
	mDataBuffer.push_back(itemTitle + item);
	++mItemCounts.back();
}
