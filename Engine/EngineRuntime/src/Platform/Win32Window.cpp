...
namespace
{
	...
	LRESULT CALLBACK WindowProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp)
	{
		...
		if (msg == WM_MOUSEWHEEL)
			wheel += static_cast<float>(GET_WHEEL_DELTA_WPARAM(wp));
		...
		if (state && state->callback)
		{
			...
			if (msg == WM_NCCREATE)
			{
				...
				result = state->callback(hwnd, msg, wp, lp);
			return result;
		}
		if (msg == WM_CLOSE)
		{
...
extern "C" HWND FtCreateNativeWindow(const char* title, unsigned int width, unsigned int height, WNDPROC callback, void* user)
{
	...
	wc.hInstance	 = GetModuleHandleW(L"EngineRuntime.dll");
	...
	HWND handle = CreateWindowExW(0, ClassName, wide.c_str(), WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, rect.right - rect.left, rect.bottom - rect.top, nullptr, nullptr, wc.hInstance, state.get());
	if (!handle)
		return nullptr;
...
{
	while (!windows.empty())
		FtDestroyNativeWindow(windows.back()->handle);
