...
void FTFont::Render(FTDS::String& text, FoxtrotRenderer* renderer, Transform* transform, Camera* camInst, FTTexture* tex, FTVertexShader* vs, FTPixelShader* ps, FTMaterial* mat)
{
	if (!Meshes())
		return;
...
void FTFont::UpdateTextVertices(FTDS::String& text, FoxtrotRenderer* renderer, FTVector2 pos, FTVector2 scale, FTVector2 padding, FTVector3 color, float alpha)
{
	if (text.IsEmpty())
		return;
	...
	for (size_t i = 0; i < text.GetLength(); ++i)
	{
		...
		vertices[i].pos		 = DirectX::XMFLOAT4(x + ((fc->OffsetX() + kerning) * scale.x), y - (fc->OffsetY() * scale.y), fc->WidthOnScreen() * scale.x, fc->HeightOnScreen() * scale.y);
		vertices[i].color	 = DirectX::XMFLOAT4(color.x, color.y, color.z, alpha);
		vertices[i].texCoord = DirectX::XMFLOAT4(fc->U(), fc->V(), fc->WidthOnTex(), fc->HeightOnTex());
...
FTFont::FTFont(FTResourceDef& resDef, FoxtrotRenderer* renderer)
	: FTMeshGroup(resDef, renderer, nullptr)
	, mSize(0)
	, mLineHeight(0.f)
	, mBaseHeight(0.f)
	, mNumCharacters(0)
	, mCharList(DBG_NEW FTDS::HashMap<FontChar*>)
	, mNumKernings(0)
	, mKerningsList(DBG_NEW FTDS::HashMap<FontKerning*>)
	, mLeftPadding(0.f)
	, mTopPadding(0.f)
	, mRightPadding(0.f)
	, mBottomPadding(0.f)
...
void FTFont::LoadProperties(std::ifstream& ifs)
{
	...
	mFontImage = ResourceManager::GetInstance()->GetLoadedTexture(key);
}

void FTFont::Process(ComPtr<ID3D11Device>& device)
{
	if (IsProcessed())
		return;

	FTDS::String metaPath = GetRelativePath();
...
void FTFont::LoadFont(FTTexture* img)
{
	FTVector2 renderRes = Camera::GetInstance()->GetResolution();

	std::wifstream fs;
	fs.open(GetRelativePath().C_Str());
