...
auto GetMemberFuncPtr(const ObjectLayout* layout, const char* name)
	-> Signature ClassType::*
{
	...
	{
		const MemberDesc& m = layout->Members[i];
...
typedef const ObjectLayout* (*GetLayoutFn)();
typedef void (*PrintStateFn)();

// -------------------------------------------------------
// Helper: find a member by name
// -------------------------------------------------------
inline const MemberDesc* FindMember(const ObjectLayout* layout, const char* name)
{
	for (size_t i = 0; i < layout->MemberCount; i++)
	{
		if (strcmp(layout->Members[i].Name, name) == 0)
			return &layout->Members[i];
	}
	return nullptr;
}
...
CORE_API const ObjectLayout* Plugin_GetLayout();
