...
BGSpriteComponent::BGSpriteComponent(Actor *owner, int drawOrder,
									 int updateOrder)
	: SpriteRendererComponent(owner, drawOrder), mScrollSpeed(0.0f) {}
	...
	/*
		...
		Alternative for
			for (auto& bg : mBGTextures)
			{
				...
				r.x =
	   static_cast<int>(GetOwner()->GetTransform()->GetWorldPosition().x - r.w /
	   2 + bg.mOffset.x); r.y =
	   static_cast<int>(GetOwner()->GetTransform()->GetWorldPosition().y - r.h /
	   2 + bg.mOffset.y);
