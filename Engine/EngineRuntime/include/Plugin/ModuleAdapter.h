...
namespace Foxtrot
{
	...
	struct PluginAdapter
	{
		...
		static Status FT_CALL Initialize(void* ptr, const HostServices*) noexcept
		{
			...
			{
				FtLog("Plugin initialization failed");
				return Status::Failed;
			}
		}
		...
		static void* FT_CALL Query(void* ptr, const char* name) noexcept
		{
			...
			if (!name || !self->Plugin)
				return nullptr;
			if (std::strcmp(name, "LegacyPlugin") == 0)
				return self->Plugin;
	...
	{
		if (!out || abi != ModuleAbi || bytes != sizeof(*out))
			return Status::AbiMismatch;
		try
		{
			auto state = new PluginAdapter{ nullptr, create, false };
			*out	   = { sizeof(*out), ModuleAbi, BuildAbi, sizeof(void*), name, state, PluginAdapter::Initialize, PluginAdapter::Shutdown, PluginAdapter::Destroy, PluginAdapter::Query };
			return Status::Ok;
		}
		catch (...)
		{
